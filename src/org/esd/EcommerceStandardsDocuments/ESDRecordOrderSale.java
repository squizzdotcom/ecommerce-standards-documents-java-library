...
public class ESDRecordOrderSale   
{
    ...
    */
    ...
    /**
    * Code of the location that the order's products are to be picked/packed/delivered/managed from by default.
    */
    public String locationCode = new String();

    /**
    * Name of the location that the order's products are to be picked/packed/delivered/managed from by default.
    */
    ...
    */
    ...
    public ESDRecordCustomerAccountEnquiryOrderSale convertToCustomerAccountEnquiryOrderSaleRecord()
    {
        ...
        ArrayList<ESDRecordCustomerAccountEnquiryOrderSaleLine> recordLines = new ArrayList<ESDRecordCustomerAccountEnquiryOrderSaleLine>();
        ...
        if(accountEnquiryOrderSaleRecord.referenceType.equalsIgnoreCase(ESDocumentCustomerAccountEnquiry.RECORD_TYPE_ORDER_PURCHASE)){
        ...
        for(int i=0; i < lines.size(); i++)
        {
            ...
            recordLine.lineType = lines.get(i).lineType;
            recordLine.referenceLineCode = lines.get(i).purchaseOrderLineCode;
            recordLine.referenceLineItemCode = lines.get(i).purchaseOrderLineNumber;
            recordLine.locationCode = lines.get(i).locationCode;
            recordLine.locationCode = lines.get(i).locationName;
            recordLine.keyLocationID = lines.get(i).keyLocationID;
            recordLine.UNSPSC = lines.get(i).UNSPSC;
            recordLine.language = lines.get(i).language;
            recordLine.quantityOrdered = lines.get(i).quantity;
            recordLine.priceExTax = lines.get(i).priceExTax;
            recordLine.priceIncTax = lines.get(i).priceIncTax;
            recordLine.priceTax = lines.get(i).priceTax;
            recordLine.totalPriceExTax = lines.get(i).priceTotalExTax;
            recordLine.totalPriceIncTax = lines.get(i).priceTotalIncTax;
            recordLine.totalPriceTax = lines.get(i).priceTotalTax;
            recordLine.unit = lines.get(i).unitName;
            recordLine.referenceLineCode = lines.get(i).priceReferenceCode;
            
            if(lines.get(i).lineType.equalsIgnoreCase(ESDocumentConstants.ORDER_LINE_TYPE_DOWNLOAD)){
            ...
            }else if(lines.get(i).lineType.equalsIgnoreCase(ESDocumentConstants.ORDER_LINE_TYPE_TEXT)){
				recordLine.description = lines.get(i).textDescription;
			}else{
                recordLine.lineItemCode = lines.get(i).productCode;
                recordLine.description = lines.get(i).productDescription;
                recordLine.referenceLineItemCode = lines.get(i).purchaseOrderProductCode;
            }
            
            if(lines.get(i).taxes != null && lines.get(i).taxes.size() > 0)
            {
                recordLine.taxCode = lines.get(i).taxes.get(0).taxcode;
            }
        ...
        accountEnquiryOrderSaleRecord.lines = recordLines.toArray(new ESDRecordCustomerAccountEnquiryOrderSaleLine[recordLines.size()]);
