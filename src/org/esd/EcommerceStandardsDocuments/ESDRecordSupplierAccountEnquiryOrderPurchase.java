...
public class ESDRecordSupplierAccountEnquiryOrderPurchase   
{
    ...
    */
	 ...
	 * @returns purchase order record and its lines
	 */
	public ESDRecordOrderPurchase convertToOrderPurchaseRecord()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			ArrayList<ESDRecordOrderLineTax> taxes = new ArrayList<ESDRecordOrderLineTax>();
			...
			{ 
				...
				//add tax record
				if(lines[i].taxCode != null && !lines[i].taxCode.trim().isEmpty())
				{
			...
			orderLine.productDeliveries = new ArrayList<ESDRecordOrderProductDelivery>();
			orderLine.attributes = new ArrayList<ESDRecordOrderLineAttributeProfile>();
		...
		orderPurchaseRecord.payments = new ArrayList<ESDRecordOrderPayment>();
		orderPurchaseRecord.surcharges = new ArrayList<ESDRecordOrderSurcharge>();
		...
		if (referenceType.equalsIgnoreCase(ESDocumentConstants.RECORD_LINE_TYPE_ORDER_SALE))
		{
		...
		orderPurchaseRecord.totalLines = lines.length;
